...
for b, kappa in test_loader:
    ...
    gamma_val = 0.01
    gamma = absorbing_layer(gamma_val, kappa[0, 0].cpu(), [16, 16], omega).to(device)
    ...
    # solve
    with torch.no_grad():
        x, ress_shift, shift_time = model.test(
            b, kappa, omega, max_level, gamma_val, T, Tx, Ty, LT, lam_max, alphas, restart, max_iter
        )

    print("Helmholtz iters =", len(ress)-1, "setup time = %f" % setup_time, "solve time = %f" % gmres_time, "|| Shift Laplacian iters =", len(ress_shift)-1, "shift time = %f" % shift_time)
    iters.append(len(ress)-1)
    setup_times.append(setup_time)
    solve_times.append(gmres_time)
    shift_iters.append(len(ress_shift)-1)
    ...
    ax3.semilogy(ress, label="helm", markersize=6)
    ax3.semilogy(ress_shift, label="SL", markersize=6)
    ax3.grid()
    ax3.legend()
...
# average
print("Helm iters = ", np.mean(iters), "setup time = %f" % np.mean(setup_times), "solve time = %f" % np.mean(solve_times))
print("SL iters = ", np.mean(shift_iters), "SL time = %f" % np.mean(shift_times))